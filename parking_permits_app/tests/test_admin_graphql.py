...
import parking_permits_app.decorators
from parking_permits_app.tests.factories.parking_permit import ParkingPermitFactory
from users_app.tests.factories.user import ADAdminFactory, UserFactory
...
class PermitsQueryTestCase(TestCase):
    ...
    @patch.object(
        parking_permits_app.decorators.RequestJWTAuthentication, "authenticate"
    )
    def test_return_parking_permits_list_for_ad_admin(self, mock_authenticate):
        ...
        url = reverse("parking_permits_app:admin-graphql")
        data = {
