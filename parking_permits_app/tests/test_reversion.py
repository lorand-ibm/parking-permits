...
from parking_permits_app.reversion import (
    EventType,
    FieldChangeResolver,
    get_obj_changelogs,
...
from users_app.tests.factories.user import UserFactory
...
class FieldChangeResolverTestCase(TestCase):
    ...
    def test_change_message(self):
        ...
        change_resolver = FieldChangeResolver(
            field, ParkingPermitStatus.DRAFT.value, ParkingPermitStatus.VALID.value
        )
...
class GetReversionCommentTestCase(TestCase):
    ...
    def test_get_changed_reversion_comment(self):
        with reversion.create_revision():
            permit = ParkingPermitFactory(status=ParkingPermitStatus.DRAFT.value)
        with reversion.create_revision():
            permit.status = ParkingPermitStatus.VALID.value
...
class GetObjChangeLogsTestCase(TestCase):
    def test_get_changelogs(self):
        ...
        self.assertEqual(changelogs[0]["event"], EventType.CHANGED.value)
        self.assertEqual(changelogs[1]["event"], EventType.CREATED.value)
